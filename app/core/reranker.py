...
class RerankerFactory:
    """
    Reranker 工厂（单例）。
    仅在 reranker.enabled=true 时创建 QwenNativeReranker 实例。
    未启用时不创建实例（HybridPDRetriever 中已有 enabled 判断，不会调用 get_instance）。
    """
    _instance: QwenNativeReranker | None = None

    @classmethod
    def init(cls) -> QwenNativeReranker | None:
        """
        初始化 Reranker 单例。仅在 enabled=true 时创建。
        """
        if cls._instance is not None:
            print("INIT RERANKER: Instance already exists, returning cached instance.")
            return cls._instance

        config = global_config.get("retriever", {}).get("reranker", {})
        if not config.get("enabled", False):
            print("INIT RERANKER: Reranker is disabled, skipping initialization.")
            return None

        cls._instance = QwenNativeReranker()
        print(f"INIT RERANKER: QwenNativeReranker initialized.")
        return cls._instance

    @classmethod
    def get_instance(cls) -> QwenNativeReranker:
        """获取 Reranker 单例。未初始化时抛出异常。"""
        if cls._instance is None:
            raise RuntimeError("Reranker has not been initialized. "
                               "Call RerankerFactory.init() first with enabled=true.")
        return cls._instance
